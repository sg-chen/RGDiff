...
from cdiffusion_utils import optim
...
class BaseExperiment(object):
    ...
    def log_step_metrics(self, iterations, iter_dict):
        raise NotImplementedError()
    ...
    def save_model_param_size(self):
        # Save args table
        with open(os.path.join(self.log_path, 'model_param_size.txt'), "w") as f:
            total_num_params = 0
            ...
            f.write("Total number of param is " + str(total_num_params))
            print("Total number of param is ", total_num_params)
    ...
    def checkpoint_load(self, check_path, name='checkpoint.pt'):
        checkpoint = torch.load(os.path.join(check_path, name), map_location="cpu")
        self.current_epoch = checkpoint['current_epoch']
        self.train_metrics = checkpoint['train_metrics']
        self.eval_metrics = checkpoint['eval_metrics']
        self.eval_epochs = checkpoint['eval_epochs']
        def load_state(model, dic:dict):
            model_state_dict = model.state_dict()
            pretrained = dic
            pretrained_dict = {k:v for k,v in pretrained.items() if k in model_state_dict}
            model_state_dict.update(pretrained_dict)
            return model_state_dict

        self.model.load_state_dict(load_state(self.model, checkpoint['model']))
        # for name, param in self.model.named_parameters():
        ...
        # self.optimizer.load_state_dict(load_state(self.optimizer, checkpoint['optimizer']))
    ...
    def run(self, epochs):
        ...
        for epoch in range(self.current_epoch, epochs):
            ...
            # Eval
            if self.current_epoch == 0 or (epoch + 1) % self.eval_every == 0:
