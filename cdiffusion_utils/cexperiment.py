import torch
from cdiffusion_utils.utils import get_args_table, clean_dict
...
# Experiment
from cdiffusion_utils import BaseExperiment
from cdiffusion_utils.base import DataParallelDistribution
...
def add_exp_args(parser):
    ...
    log_base = ""
    parser.add_argument('--log_home', type=str, default=log_base)
    parser.add_argument("--model_params", type=eval, default=True)
...
class DiffusionExperiment(BaseExperiment):
    ...
    def __init__(self, args,
        ...
        if args.log_home is None:
            self.log_base = os.path.join('log', 'flow')
        else:
    ...
    def log_step_metrics(self, iterations, iter_dict):
        # Tensorboard
        if self.args.log_tb:
            for metric_name, metric_value in iter_dict.items():
                self.writer.add_scalar('{}'.format(metric_name), metric_value, global_step=iterations + 1)

        # Weights & Biases
        if self.args.log_wandb:
            for metric_name, metric_value in iter_dict.items():
                wandb.log({'{}'.format(metric_name): metric_value}, step=iterations + 1)
